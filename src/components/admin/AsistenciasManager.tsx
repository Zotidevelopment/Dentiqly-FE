...
export const AsistenciasManager: React.FC = () => {
  ...
  const [selectedFreePatient, setSelectedFreePatient] = useState<any | null>(null)
  ...
  // ── Walk-in attendance (no prior appointment) ───────────────────────────
  const handleWalkIn = async (patient: any, estado: "Atendido" | "Ausente") => {
    try {
      setCreatingWalkIn(true)
      const now = new Date()
      const hh = String(now.getHours()).padStart(2, "0")
      const mm = String(now.getMinutes()).padStart(2, "0")
      const hora_inicio = `${hh}:${mm}`
      const endMin = now.getHours() * 60 + now.getMinutes() + 30
      const hora_fin = `${String(Math.floor(endMin / 60)).padStart(2, "0")}:${String(endMin % 60).padStart(2, "0")}`

      const nuevo = await turnosApi.crear({
      ...
      // Enrich and add to local state so it shows in the table immediately
      const enriched = {
        ...nuevo,
        paciente: patient,
        profesional: (nuevo as any).profesional || null,
        servicio: (nuevo as any).servicio || null,
      }
      setAppointments((prev) => [...prev, enriched])
      toast({
        title: estado === "Atendido" ? "Asistencia registrada" : "Ausencia registrada",
        description: `${patient.nombre} ${patient.apellido} marcado como ${estado}.`,
      })
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      ...
      try {
        const res = await pacientesApi.listar({ search: patientSearch, limit: 10 })
  ...
  // Find the turno for the selected free patient on the selected date (if any)
  const freePatientTurno = selectedFreePatient
    ? appointments.find(
        (a) => a.paciente_id === selectedFreePatient.id && a.fecha === selectedDate
      ) || null
    : null
  ...
  const getPeriodFilteredAppointments = () => {
    if (periodFilter === "diario") return dailyAppointments
    if (periodFilter === "semana") return appointments.filter((appt) => isSameWeek(appt.fecha, selectedDate))
    if (periodFilter === "mes") return appointments.filter((appt) => isSameMonth(appt.fecha, selectedDate))
    return appointments
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex bg-gray-100 p-0.5 rounded-xl border border-gray-200/50 shadow-xs">
            {[
              { label: "Ayer", fn: setYesterday, active: selectedDate === getYesterdayDate() },
              { label: "Hoy",  fn: setToday,     active: selectedDate === getTodayDate() },
              { label: "Mañana", fn: setTomorrow, active: selectedDate === getTomorrowDate() },
            ].map((b) => (
              <Button
                key={b.label}
                variant="ghost"
      ...
      <Card className="p-5 border-[#E8E0D6] bg-gradient-to-br from-[#EEF3FF]/60 to-white" style={{ overflow: "visible" }}>
        <div className="flex items-center gap-2 mb-4">
          ...
          <div>
            <h3 className="text-sm font-bold text-[#0B1023]">Confirmar asistencia por paciente</h3>
            <p className="text-[11px] text-gray-400">Buscá cualquier paciente aunque no esté agendado hoy</p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-start">
          ...
          <div className="relative flex-1" ref={patientSearchRef}>
              ...
              <div className="absolute top-full left-0 right-0 mt-1.5 bg-white border border-gray-100 rounded-xl shadow-lg z-50 overflow-hidden">
                {patientResults.map((p) => (
                  <button
                    key={p.id}
                    onClick={() => {
                      setSelectedFreePatient(p)
                      setPatientSearch(`${p.nombre} ${p.apellido}`)
                      setShowPatientDropdown(false)
                    }}
                    className="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-[#EEF3FF] transition-colors text-left group"
                  >
                    <div className="w-7 h-7 rounded-full bg-[#2563FF]/10 flex items-center justify-center text-[10px] font-bold text-[#2563FF] shrink-0">
            ...
            {showPatientDropdown && patientResults.length === 0 && !patientSearchLoading && patientSearch.length >= 2 && (
              <div className="absolute top-full left-0 right-0 mt-1.5 bg-white border border-gray-100 rounded-xl shadow-lg z-50 p-4 text-xs text-gray-400 text-center">
                No se encontraron pacientes
              </div>
            )}
          </div>

          {/* Action buttons once a patient is selected */}
          {selectedFreePatient && (
            <div className="flex items-center gap-2 shrink-0">
              {freePatientTurno ? (
                /* Patient IS scheduled today — show their turno's current state */
                freePatientTurno.estado === "Atendido" ? (
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => handleStatusUpdate(freePatientTurno.id, "Confirmado")}
                    disabled={updatingId === freePatientTurno.id}
                    className="h-9 px-3 text-gray-500 border-gray-200 text-xs"
                  >
                    {updatingId === freePatientTurno.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : "Revertir"}
                  </Button>
                ) : (
                  <>
                    <Button
                ...
                /* Patient NOT scheduled — allow walk-in registration */
                <>
                  <Button
                    size="sm"
              ...
              {/* Clear selection */}
              <button
                onClick={() => { setSelectedFreePatient(null); setPatientSearch("") }}
                className="h-9 w-9 rounded-xl border border-gray-200 flex items-center justify-center hover:bg-gray-50 transition-colors text-gray-400"
                title="Limpiar"
              >
                <X className="h-4 w-4" />
              </button>
        ...
        {/* Selected patient info pill */}
        {selectedFreePatient && (
          <div className="mt-3 flex items-center gap-3 bg-white border border-gray-100 rounded-xl px-4 py-2.5 text-sm">
            <div className="w-8 h-8 rounded-full bg-[#2563FF]/10 flex items-center justify-center text-[11px] font-bold text-[#2563FF] shrink-0">
              {selectedFreePatient.nombre?.[0]}{selectedFreePatient.apellido?.[0]}
            </div>
            <div>
        ...
        <Card className="p-5 border-[#E8E0D6] flex flex-col h-fit">
          <div className="flex flex-col gap-3 pb-4 border-b border-[#E8E0D6]/60 mb-4">
                ...
                {(["diario", "semana", "mes"] as const).map((f) => (
                  <button
                    ...
                    className={`px-2.5 py-1 rounded-md text-[10px] font-bold transition-all cursor-pointer ${
                      periodFilter === f ? "bg-white text-[#2563FF] shadow-xs" : "text-gray-500 hover:text-gray-900 bg-transparent"
                    }`}
                  >
                    {f === "diario" ? "Día" : f === "semana" ? "Sem." : "Mes"}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="flex bg-gray-50 p-1 rounded-xl border border-gray-200/40 shadow-inner">
              {(["obra_social", "servicio"] as const).map((t) => (
                <button
                  key={t}
                ...
                >
                  {t === "obra_social" ? "Por Obra Social" : "Por Servicio"}
                </button>
              ))}
            </div>
          </div>
          ...
          {activeStats.length > 0 && (
            <div className="mb-3 text-[10px] text-gray-400 font-semibold uppercase tracking-wider">
              {periodFilter === "diario" ? "Período: Diario" : periodFilter === "semana" ? "Período: Semanal" : "Período: Mensual"}
            </div>
          )}
                    ...
                    return (
                      <tr key={appt.id} className="hover:bg-gray-50/60 transition-colors">
                        ...
                        <td className="px-4 py-3.5 whitespace-nowrap">
                          <div className="font-semibold text-gray-800">{appt.paciente?.nombre} {appt.paciente?.apellido}</div>
                        ...
                        <td className="px-4 py-3.5 text-right whitespace-nowrap">
                          <div className="flex justify-end gap-1.5">
                            {isAttended ? (
                              ...
                              >
                                {updatingId === appt.id ? <Loader2 className="h-3 w-3 animate-spin" /> : "Revertir"}
                              </Button>
                            ) : (
                              <>
                                <Button
      ...
      {/* ═══════════════════════════════════════════════════════════════════
          SECTION 3 — STATS CARDS (at the bottom)
      ════════════════════════════════════════════════════════════════════ */}
      <div>
